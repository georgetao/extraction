...
def get_enc_seg_data(batch):
    batch_size = len(batch.enc_lens)
    enc_batch = T.from_numpy(batch.enc_batch).long()
    enc_seg_batch = T.from_numpy(batch.enc_seg_batch).long()
    enc_padding_mask = T.from_numpy(batch.enc_padding_mask).float()
    ...
    enc_batch = get_cuda(enc_batch)
    enc_seg_batch = get_cuda(enc_seg_batch)
    enc_padding_mask = get_cuda(enc_padding_mask)
    ...
    return enc_batch, enc_seg_batch, enc_lens, enc_padding_mask, enc_batch_extend_vocab, extra_zeros, ct_e
